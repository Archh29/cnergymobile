...
switch($action) {
    ...
    case 'get_muscle_parts':
        getMuscleParts($pdo);
        break;
    case 'fetchMusclesWithParts':
        fetchMusclesWithParts($pdo);
        break;
...
function getExercises($pdo) {
    try {
        ...
        foreach ($exercises as &$exercise) {
            ...
            $exercise['target_muscles'] = $muscleStmt->fetchAll(PDO::FETCH_ASSOC);
        }
    ...
    try {
        ...
        foreach ($muscles as &$muscle) {
            ...
            if (!empty($muscle['image_url'])) {
                ...
                // Check if this is already a full URL to image-servers.php
                if (strpos($imageUrl, 'image-servers.php') !== false) {
                    parse_str(parse_url($imageUrl, PHP_URL_QUERY), $params);
                    $filename = $params['image'] ?? '';
                    
                    // If the image parameter is invalid (like 'image-servers.php' itself), use fallback
                    if (empty($filename) || strpos($filename, '.php') !== false || $filename === 'image-servers.php') {
                        $muscle['image_url'] = getDefaultMuscleImage($muscle['name']);
                    } else {
                        // Keep the existing URL if it has a valid image parameter
                        $muscle['image_url'] = 'https://api.cnergy.site/image-servers.php?image=' . $filename;
                    }
                } elseif (strpos($imageUrl, 'localhost') !== false || strpos($imageUrl, '127.0.0.1') !== false) {
                    ...
                    if (!empty($filename) && $filename !== 'image-servers.php' && strpos($filename, '.php') === false) {
                        $muscle['image_url'] = 'https://api.cnergy.site/image-servers.php?image=' . $filename;
                    } else {
                        $muscle['image_url'] = getDefaultMuscleImage($muscle['name']);
                    }
                }
                // If it's already a proper https URL, keep it
            } else {
                // Use default image if none is set
                $muscle['image_url'] = getDefaultMuscleImage($muscle['name']);
            }
            
            error_log("DEBUG: Muscle {$muscle['name']} final image_url: " . ($muscle['image_url'] ?? 'NULL'));
        }
...
// ============================================
// NEW FUNCTION FOR PRIMARY MUSCLE PARTS LOGIC
// ============================================
function fetchMusclesWithParts($pdo) {
    try {
        // Get all muscle groups (parent_id is NULL)
        $muscleGroupsQuery = "SELECT id, name, image_url FROM target_muscle WHERE parent_id IS NULL ORDER BY name";
        $muscleGroupsResult = $pdo->query($muscleGroupsQuery);
        
        if (!$muscleGroupsResult) {
            throw new Exception('Failed to fetch muscle groups: ' . $pdo->errorInfo()[2]);
        }
        
        $muscleGroups = [];
        
        while ($muscleGroup = $muscleGroupsResult->fetch(PDO::FETCH_ASSOC)) {
            // Get primary parts for this muscle group (parent_id = muscle group id)
            $primaryPartsQuery = "SELECT id, name, image_url FROM target_muscle WHERE parent_id = ? ORDER BY name";
            $stmt = $pdo->prepare($primaryPartsQuery);
            $stmt->execute([$muscleGroup['id']]);
            $primaryPartsResult = $stmt->fetchAll(PDO::FETCH_ASSOC);
            
            $primaryParts = [];
            foreach ($primaryPartsResult as $primaryPart) {
                $primaryParts[] = [
                    'id' => (int)$primaryPart['id'],
                    'name' => $primaryPart['name'],
                    'description' => '', // Add description if needed
                ];
            }
            
            // Fix image URL if needed
            $imageUrl = $muscleGroup['image_url'] ?? '';
            if (!empty($imageUrl)) {
                if (strpos($imageUrl, 'localhost') !== false || strpos($imageUrl, '127.0.0.1') !== false) {
                    ...
                    if (!empty($filename)) {
                        $imageUrl = 'https://api.cnergy.site/image-servers.php?image=' . $filename;
                    } else {
                        $imageUrl = getDefaultMuscleImage($muscleGroup['name']);
                    }
                } elseif (strpos($imageUrl, 'http') !== 0) {
                    $imageUrl = 'https://api.cnergy.site/image-servers.php?image=' . basename($imageUrl);
                }
            } else {
                $imageUrl = getDefaultMuscleImage($muscleGroup['name']);
            }
        ...
        echo json_encode([
            'success' => false,
            'error' => 'Error fetching muscle groups with parts: ' . $e->getMessage()
        ]);
    }
}
?>
