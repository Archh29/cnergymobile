-- =====================================================
-- COMPREHENSIVE NOTIFICATION TRIGGERS FOR CNERGY GYM APP
-- =====================================================
...
BEGIN
    ...
    VALUES (
        NEW.user_id, 
        CONCAT('🏆 NEW PERSONAL RECORD! ', NEW.exercise_name, ': ', NEW.weight, ' lbs x ', NEW.reps, ' reps'),
        1, 
        6, 
        NOW()
    );
...
BEGIN
    IF NEW.created_by IS NOT NULL AND NEW.created_by != NEW.user_id THEN
        ...
        VALUES (
            NEW.user_id, 
            CONCAT('📋 New workout program assigned by ', coach_name, ': ', NEW.goal),
            1, 
            7, 
            NOW()
        );
...
-- Trigger for goal deadline approaching (7 days)
DELIMITER $$
...
BEGIN
    IF NEW.target_date IS NOT NULL AND DATEDIFF(NEW.target_date, CURDATE()) = 7 THEN
        ...
        VALUES (
            NEW.user_id, 
            CONCAT('⏰ Goal deadline approaching: ', NEW.goal_name, ' in 7 days!'),
            1, 
            5, 
            NOW()
        );
