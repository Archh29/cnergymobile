...
class _ProgressiveOverloadTrackerState extends State<ProgressiveOverloadTracker>
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      for (final program in detailedPrograms) {
        ...
        if (programId != null) {
          // Try to get progress from progress tracker API first
          List<ProgressTrackerModel> progress = [];
          try {
            progress = await ProgressAnalyticsService.getProgressByProgram(programId);
            print('✅ Got progress from API for program $programId: ${progress.length} records');
  ...
  void _showExerciseLogs(String exerciseName) async {
    ...
    print('🔍 Program ID: $programId');
     ...
     // Get the real workout data from API (same as the summary uses)
     final allProgressData = await ProgressAnalyticsService.getAllProgress();
     List<ProgressTrackerModel> exerciseLogs = [];
     
     // Find the exercise data from the API response
     for (final entry in allProgressData.entries) {
       if (entry.key == exerciseName) {
         exerciseLogs = entry.value;
         break;
       }
     }
     ...
     if (exerciseLogs.isEmpty) {
       ...
       print('🔍 EXERCISE LOGS: Available exercises in API: ${allProgressData.keys.toList()}');
     }
