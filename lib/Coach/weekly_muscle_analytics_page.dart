...
class WeeklyMuscleAnalyticsPage extends StatefulWidget {
  ...
  const WeeklyMuscleAnalyticsPage({Key? key, this.selectedMember}) : super(key: key);
...
class _WeeklyMuscleAnalyticsPageState extends State<WeeklyMuscleAnalyticsPage> {
  ...
  int? get _memberId {
    if (widget.selectedMember != null) {
      ...
      if (widget.selectedMember is Map) {
        return widget.selectedMember['id'] ?? widget.selectedMember['user_id'];
      } else {
        // MemberModel
        return widget.selectedMember.id;
      }
    }
    // Fallback to current user
    return null; // Let service use default
  }
  ...
  Future<void> _load() async {
    ...
    try {
      final data = await WeeklyMuscleAnalyticsService.getWeekly(
